...
/* External variables --------------------------------------------------------*/
 ...
 extern uint8_t FrameReceived; /* Flag to indicate CAN frame reception */
 extern bool ReceptionEnable;
 extern bool LINSlave;

 /* External functions --------------------------------------------------------*/
extern void SetLINTimebaseTick(void);
extern void SetCANTimebaseTick(void);
    ...
    */
	...
	if (LINSlave==TRUE)
	{
		...
		if (UART3->CR6 & UART3_CR6_LHDF)
		{
			...
			HeaderReceived=1;
			DataReceived=0; /* remove limitation when mute mode disabled and del lentgh=23.9-bit */
			IdentifierParityError = UART3_SR_Buf&0x01;		
		}
		#else
		...
		else
		{
			#ifdef LINUART
			/* Header error ? */
			if (UART3_SR_Buf & UART3_SR_OR) /* UART3_SR_OR is same bit as UART3_SR_LHE */
			{			
