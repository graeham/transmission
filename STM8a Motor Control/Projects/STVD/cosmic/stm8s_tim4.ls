...
1288  01e8 2001          	jra	L574
1289  01ea               L374:
1290                     ; 331     bitstatus = RESET;
1292  01ea 4f            	clr	a
1294  01eb               L574:
...
1336                     ; 344 {
1337                     	switch	.text
1338  01ed               _TIM4_ClearFlag:
1340  01ed 88            	push	a
1341       00000000      OFST:	set	0
1344                     ; 346     assert_param(IS_TIM4_GET_FLAG_OK(TIM4_FLAG));
1346  01ee 4a            	dec	a
1347  01ef 270e          	jreq	L002
1348  01f1 ae015a        	ldw	x,#346
1349  01f4 89            	pushw	x
1350  01f5 5f            	clrw	x
1351  01f6 89            	pushw	x
1352  01f7 ae0000        	ldw	x,#L56
1353  01fa cd0000        	call	_assert_failed
1355  01fd 5b04          	addw	sp,#4
1356  01ff               L002:
1357                     ; 349     TIM4->SR1 = (uint8_t)(~TIM4_FLAG);
1359  01ff 7b01          	ld	a,(OFST+1,sp)
1360  0201 43            	cpl	a
1361  0202 c75342        	ld	21314,a
1362                     ; 351 }
1365  0205 84            	pop	a
1366  0206 81            	ret	
1427                     ; 360 ITStatus TIM4_GetITStatus(TIM4_IT_TypeDef TIM4_IT)
1427                     ; 361 {
1428                     	switch	.text
1429  0207               _TIM4_GetITStatus:
1431  0207 88            	push	a
1432  0208 89            	pushw	x
1433       00000002      OFST:	set	2
1436                     ; 362     ITStatus bitstatus = RESET;
1438                     ; 364   uint8_t itstatus = 0x0, itenable = 0x0;
1442                     ; 367   assert_param(IS_TIM4_IT_OK(TIM4_IT));
1444  0209 4a            	dec	a
1445  020a 270e          	jreq	L012
1446  020c ae016f        	ldw	x,#367
1447  020f 89            	pushw	x
1448  0210 5f            	clrw	x
1449  0211 89            	pushw	x
1450  0212 ae0000        	ldw	x,#L56
1451  0215 cd0000        	call	_assert_failed
1453  0218 5b04          	addw	sp,#4
1454  021a               L012:
1455                     ; 369   itstatus = (uint8_t)(TIM4->SR1 & (uint8_t)TIM4_IT);
1457  021a c65342        	ld	a,21314
1458  021d 1403          	and	a,(OFST+1,sp)
1459  021f 6b01          	ld	(OFST-1,sp),a
1461                     ; 371   itenable = (uint8_t)(TIM4->IER & (uint8_t)TIM4_IT);
1463  0221 c65341        	ld	a,21313
1464  0224 1403          	and	a,(OFST+1,sp)
1465  0226 6b02          	ld	(OFST+0,sp),a
1467                     ; 373   if ((itstatus != (uint8_t)RESET ) && (itenable != (uint8_t)RESET ))
1469  0228 7b01          	ld	a,(OFST-1,sp)
1470  022a 2708          	jreq	L345
1472  022c 7b02          	ld	a,(OFST+0,sp)
1473  022e 2704          	jreq	L345
1474                     ; 375     bitstatus = (ITStatus)SET;
1476  0230 a601          	ld	a,#1
1479  0232 2001          	jra	L545
1480  0234               L345:
1481                     ; 379     bitstatus = (ITStatus)RESET;
1483  0234 4f            	clr	a
1485  0235               L545:
1486                     ; 381   return ((ITStatus)bitstatus);
1490  0235 5b03          	addw	sp,#3
1491  0237 81            	ret	
1528                     ; 391 void TIM4_ClearITPendingBit(TIM4_IT_TypeDef TIM4_IT)
1528                     ; 392 {
1529                     	switch	.text
1530  0238               _TIM4_ClearITPendingBit:
1532  0238 88            	push	a
1533       00000000      OFST:	set	0
1536                     ; 394     assert_param(IS_TIM4_IT_OK(TIM4_IT));
1538  0239 4a            	dec	a
1539  023a 270e          	jreq	L022
1540  023c ae018a        	ldw	x,#394
1541  023f 89            	pushw	x
1542  0240 5f            	clrw	x
1543  0241 89            	pushw	x
1544  0242 ae0000        	ldw	x,#L56
1545  0245 cd0000        	call	_assert_failed
1547  0248 5b04          	addw	sp,#4
1548  024a               L022:
1549                     ; 397     TIM4->SR1 = (uint8_t)(~TIM4_IT);
1551  024a 7b01          	ld	a,(OFST+1,sp)
1552  024c 43            	cpl	a
1553  024d c75342        	ld	21314,a
1554                     ; 398 }
1557  0250 84            	pop	a
1558  0251 81            	ret	
