...
void LCD_GLASS_Init(void)
{
 ...
 /* Enable LCD/RTC clock */
  ...
  in the Discovery we use some of 0 to 23 segments. */
  LCD_PortMaskConfig(LCD_PortMaskRegister_0, 0xF3);
  LCD_PortMaskConfig(LCD_PortMaskRegister_1, 0x3F);
  LCD_PortMaskConfig(LCD_PortMaskRegister_2, 0x3C);
  
  /* To set contrast to mean value */
  LCD_ContrastConfig(LCD_Contrast_2V8);
...
/* To convert displayed character in segment in array digit */
  ...
  switch (position)
  {
    ...
    case 2:
      ...
      LCD->RAM[LCD_RAMRegister_2] |= (uint8_t)(digit[0]<<2) & 0x30; // 4G 4B
      LCD->RAM[LCD_RAMRegister_4] |= (uint8_t)(digit[1]<<2) & 0x0C; // 4C 4D
      LCD->RAM[LCD_RAMRegister_6] |= (uint8_t)(digit[1]>>2) & 0x03; // 4F 4A
      LCD->RAM[LCD_RAMRegister_7] |= (uint8_t)(digit[2]<<6) & 0xC0; // 4Col 4P		
			LCD->RAM[LCD_RAMRegister_9] |= (uint8_t)(digit[2]<<2) & 0x30 ; // 4Q 4K										
      LCD->RAM[LCD_RAMRegister_11] |= (uint8_t)(digit[3]) & 0x0C; // 4DP 4N	
      LCD->RAM[LCD_RAMRegister_13] |= (uint8_t)(digit[3]) & 0x03; // 4H 4J
      break;
    ...
    case 3:
      ...
      LCD->RAM[LCD_RAMRegister_0] |=  (uint8_t)(digit[0]>>2) & 0x03; // 5G 5B			
      LCD->RAM[LCD_RAMRegister_1] |=  (uint8_t)(digit[0]) & 0x03; // 5M 5E	
      LCD->RAM[LCD_RAMRegister_3] |=  (uint8_t)(digit[1]<<2) & 0x30; // 5F 5A
			LCD->RAM[LCD_RAMRegister_4] |= (uint8_t)(digit[1]<<4) & 0x30; // 5C 5D
      LCD->RAM[LCD_RAMRegister_7] |=  (uint8_t)(digit[2]>>2) & 0x03; // 5Q 5K	
			// 5 DP 5 COL not used
      LCD->RAM[LCD_RAMRegister_8] |=  (uint8_t)(digit[2]) & 0x01; //  5P				
      LCD->RAM[LCD_RAMRegister_10] |=  (uint8_t)(digit[3]<<4) & 0x30; // 5H 5J
			LCD->RAM[LCD_RAMRegister_11] |=  (uint8_t)(digit[3]<<2) & 0x10; // 5N	
      break;
    
    /* Position 6 on LCD (Digit6)*/
    case 4:
      LCD->RAM[LCD_RAMRegister_1] &= 0xc3;
