...
//JNa If R=2K then X10 else if R=1K then X20
#define CURRENT_MUL 10L /* Rev B */
//#define CURRENT_MUL 20L /* Rev A */
...
#undef DEBUG /* Enable this constant for debugging */

extern u16 ADC_Icc_Test(u8 Mcu_State);
extern u16 ADC_Supply(void);
...
/* Switch in LSE clock */
  ...
  LCD_GLASS_ScrollSentence("    LSE OK ",1,SCROLL_SPEED); //Press reset for exit
...
{  
  ...
  Auto_test = FALSE;
	...
	LCD_GLASS_ScrollSentence("    ** TEST OK **",1,SCROLL_SPEED_H);
}
